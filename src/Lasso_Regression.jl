...
# 'alp' is the regularization parameter
function gradientDescent(X, Y, B, learningRate, numIterations, alp)
    ...
    for iteration in 1:numIterations
        ...
        gradient = ((X' * loss).+(alp.*(broadcast(abs, B)./B)))/m   # differential of mod(x) is mod(x)/x
...
CSV.write("data/2b.csv",DataFrame(YPred), writeheader=false)
